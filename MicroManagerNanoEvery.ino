 ...
 */
 ...
 void setup() {
   ...
   // Not sure why we do this.. but it was in the original code.
   //PORTB = ((int) PORTB) & 0xFF;
    PORTC.OUT &= (1 << 6);    // UP port C6.
    PORTF.OUT &= (1 << 4);    // UP port F4.
    PORTA.OUT &= (1 << 1);    // UP port A1.
    PORTE.OUT &= (1 << 3);    // UP port E3.
    PORTB.OUT &= (1 << 0);    // UP port B0.
    PORTB.OUT &= (1 << 1);    // UP port B1.
   
   // Turn on build-in pull-up resistors
   //PORTC = PORTC | B00111111;
   PORTD.OUT |= B00111111;

    currentPattern_ = B00011000;
   setDigitalOut(currentPattern_);
...
/*
 *  Since we cannot write to a single register on the Arduino Zero Every,
 *  this writes to the various registers in turn. Not as fast, but at
 *  least the port numbers make some form of sense.
 */
void setDigitalOut(byte pattern) {
    PORTC.OUT = (bitRead(pattern, 0) << 6);    // Set port C6
    PORTF.OUT = (bitRead(pattern, 1) << 4);    // Set port F4
    PORTA.OUT = (bitRead(pattern, 2) << 1);    // Set port A1
    PORTE.OUT = (bitRead(pattern, 3) << 3);    // Set port E3
    PORTB.OUT = (bitRead(pattern, 4) << 0);    // Set port B0
    PORTB.OUT = (bitRead(pattern, 5) << 1);    // Set port B1
}

byte getDigitalOutput () {
    byte output = 0;
    bitWrite(output, 0, bitRead(PORTC.OUT, 6)); // Read port C6
    bitWrite(output, 1, bitRead(PORTF.OUT, 4)); // Read port F4
    bitWrite(output, 2, bitRead(PORTA.OUT, 1)); // Read port A1    
    bitWrite(output, 3, bitRead(PORTE.OUT, 3)); // Read port E3
    bitWrite(output, 4, bitRead(PORTB.OUT, 0)); // Read port B0
    bitWrite(output, 5, bitRead(PORTB.OUT, 1)); // Read port B1
    return output;            
}
 ...
 void loop() {
   if (Serial.available() > 0) {
     ...
     switch (inByte) {
       ...
       case 2:
          Serial.write( byte(2));
          //Serial.write( PORTB.OUT);
          Serial.write( getDigitalOutput() );
          break;
...
//           PORTB = B00000000;
           setDigitalOut(B00000000); // Turn off all digital outs.
...
//           PORTB = B00000000;
    ...
    if (triggerMode_) {
      boolean tmp = PORTD.IN & inPinBit_;
      if (tmp != triggerState_) {
        if (blankOnHigh_ && tmp ) {
          ...
          if (triggerNr_ >=0) {
            //PORTB.OUT = triggerPattern_[sequenceNr_];
            setDigitalOut(triggerPattern_[sequenceNr_]); // Set output to current sequence pattern
            sequenceNr_++;
...
// OLD analogue output. Replaced with above ^
/** 
 * void analogueOut(int channel, byte msb, byte lsb) 
{
